...
import { 
  ...
  FileSpreadsheet, Bell, LogOut, Settings, 
  ShieldCheck, MapPin, Users
} from 'lucide-react';
import { useAuth } from '@/context/AuthContext';

const DISTRICT_LINKS = [
  { name: 'District Portal', href: '/district', icon: LayoutDashboard },
];
      ...
      <aside className="hidden md:flex flex-col w-72 bg-white border-r border-gray-200 shadow-[4px_0_24px_rgba(0,0,0,0.02)] fixed h-full z-20">
        <div className="p-6 flex items-center space-x-3 border-b border-gray-100 h-24 bg-linear-to-b from-gray-50 to-white">
          ...
          <div>
            ...
            <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest mt-1">District Command</p>
          </div>
        </div>

        <nav className="flex-1 overflow-y-auto py-6 px-4 space-y-1.5">
          {DISTRICT_LINKS.map((link) => {
      ...
      <main className="flex-1 md:ml-72 flex flex-col min-h-screen relative overflow-y-auto">
        ...
        <header className="hidden md:flex h-24 bg-white/90 backdrop-blur-xl border-b border-gray-200 items-center justify-between px-10 sticky top-0 z-30 shadow-sm">
          <div className="flex items-center">
            ...
            <span className="text-xs font-black text-gray-500 uppercase tracking-widest">
              District Jurisdiction: <span className="text-[#1E3A8A]">{profile?.district || "South West Delhi"}</span>
            </span>
          </div>
              ...
              {isOpen && (
                ...
                >
                  <div className="p-5 bg-linear-to-r from-[#1E3A8A] to-[#0f172a] text-white border-b border-gray-800">
                    <span className="text-[10px] font-black uppercase tracking-widest text-[#FF9933] bg-[#FF9933]/20 px-2 py-0.5 rounded-sm mb-1.5 inline-block border border-[#FF9933]/30">District Access</span>
                    <p className="text-base font-black">{profile?.fullName || "District Manager"}</p>
                    <p className="text-xs text-blue-200 truncate">{profile?.district || "South West Delhi"}</p>
                  </div>
        ...
        <header className="md:hidden fixed top-0 w-full h-16 bg-white border-b border-gray-200 shadow-sm z-40 px-4 flex justify-between items-center">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-black text-[#1E3A8A] tracking-tight">District Portal</span>
          </div>
          <button 
            onClick={() => setIsOpen(!isOpen)}
